...
class FeedsDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        super(
            FeedsDialog, self).__init__(
            parent,
            title=_(
                "Feeds: {defaultFeed} ({configProfile})".format(
    ...
    def onSearchEditTextChange(self, evt):
        ...
        try:
            ...
            for control in (self.feedsList, self.articlesButton, self.openButton, self.renameButton, self.deleteButton,
                            self.defaultButton):
                control.disable
...
class ArticlesDialog(wx.Dialog):
    ...
    def onArticlesListInfo(self, evt):
        articleInfo = "{title}\r\n\r\n{address}".format(
            title=self.Parent.feed.getArticleTitle(self.articlesList.Selection),
...
class CopyDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        groupHelper = sHelper.addItem(gui.guiHelper.BoxSizerHelper(
            self, sizer=wx.StaticBoxSizer(
                wx.StaticBox(self, label=directoryGroupText),
                wx.VERTICAL)))
        # Message translated in NVDA core.
...
class Feed(object):
    ...
    def refresh(self):
        try:
            ...
            self._document = ElementTree.parse(urllib.request.urlopen(req))
        except Exception as e:
            raise e
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    @script(
        # Translators: message presented in input mode.
        description=_(
            "Announces the title of the current article. Pressed two times, copies title and related link to the clipboard."),
    ...
    def script_readCurrentArticle(self, gesture):
        if not self.feed:
            self.getFirstArticle()
        articleInfo = "{title}\r\n\r\n{address}".format(
            title=self.feed.getArticleTitle(),
